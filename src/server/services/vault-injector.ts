...
export function buildVaultBlock(req: VaultInjectionRequest): VaultInjectionResult | null {
  ...
  let files: { relativePath: string; tags: string[]; content: string }[];
  ...
  lines.push('You have access to the following project wiki files. Treat each <wiki_file> as authoritative reference material curated by the user. Apply it where relevant. Wikilinks in the form [[Title]] reference other files in this collection.');
  lines.push('');

  for (const f of files) {
    const pathAttr = escapeAttr(f.relativePath);
    const tagsAttr = f.tags.length > 0 ? ` tags="${escapeAttr(f.tags.join(','))}"` : '';
    lines.push(`<wiki_file path="${pathAttr}"${tagsAttr}>`);
    lines.push(f.content);
    lines.push('</wiki_file>');
  }
...
function readFileContents(
  projectRoot: string,
  paths: string[],
): { relativePath: string; tags: string[]; content: string }[] {
  const resolved = path.resolve(projectRoot);
  const results: { relativePath: string; tags: string[]; content: string }[] = [];
  ...
  for (const rel of paths) {
    ...
    results.push({ relativePath: rel.replace(/\\/g, '/'), tags, content });
  }
