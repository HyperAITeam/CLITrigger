...
function getJiraConfig(projectId: string): JiraConfig | null {
  ...
  if (!project || !project.jira_enabled || !project.jira_base_url || !project.jira_email || !project.jira_api_token) {
    return null;
  }
  return {
    baseUrl: project.jira_base_url.replace(/\/+$/, ''),
...
router.get('/:projectId/test', async (req: PidReq, res: Response) => {
  const config = getJiraConfig(req.params.projectId);
  if (!config) {
