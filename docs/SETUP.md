...
- **탭 직접 이동 / pane 이동 / pane 줌**: `Ctrl+1..9`(Mac `Cmd`)로 N번째 탭 활성화(단일 탭 stack은 PTY로 fall-through). `Ctrl+Shift+←↑↓→`로 스플릿 안의 인접 pane으로 포커스 이동(xterm 포커스까지 따라감). `Ctrl+Shift+Z`로 포커스된 pane만 최대화하는 zoom 토글 — 다른 pane은 마운트 유지(display:none)라 출력 유실 없음, 재입력 시 원래 sizes 복원, 새로고침 후 미영속. 단일 stack 윈도우에서는 창 최대화 토글로 동작
- **에이전트 상태 점 + 입력 대기 알림**: 서버가 PTY 출력을 휴리스틱으로 읽어 `working`(스피너) / `blocked`(프롬프트 대기·질문·승인 다이얼로그) / `done`을 판정하고 WS `session:agent-state`로 흘려보냄(Claude CLI만 감지, 다른 CLI는 `unknown`). 탭의 상태 점이 작업 중이면 파랗게 펄스, 입력 대기면 앰버. 보이지 않는 탭(백그라운드 탭·최소화 칩·포커스 잃은 창)이 `blocked`/`done`으로 바뀌면 탭 제목이 상태 색으로 강조되고 브라우저 알림("세션이 입력을 기다립니다")이 1회 뜸 — 그 탭을 포커스하면 강조가 사라짐. 사용자가 직접 중지한 세션은 알림 대상이 아님. REST `GET /api/sessions/:id`의 `agent_state`와 MCP `wait_session_state`도 같은 값을 사용
