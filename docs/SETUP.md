...
4. 타입별로 필요한 정보 입력:
   - 실행파일/명령: **Target** (필수), **Args** (선택, 64개 항목까지), **CWD** (선택, 작업 디렉터리)
...
Planner는 CRUD + 태그 + 우선순위 기반의 경량 작업 관리 기능입니다. 아이템을 TODO 또는 스케줄로 변환하여 실제 실행 단계로 넘길 수 있습니다.
...
git이 아닌 SVN 워킹 카피로 굴러가는 프로젝트는 SVN 패널을 opt-in으로 활성화할 수 있습니다.

#### 활성화

1. 프로젝트 설정에서 **Enable SVN** 체크박스 ON (git 프로젝트에선 노출되지 않음).
...
프로젝트 상세 페이지에서 **Harness** 탭 진입. CLI별 탭(Claude / Gemini / Codex)으로 전환.
...
확장된 디렉토리, 숨김 파일 토글, 선택 파일이 프로젝트별 localStorage에 저장됩니다. 탭 전환이나 프로젝트 이동 후 돌아와도 마지막 상태가 복원됩니다. 확장된 디렉토리는 depth별로 그룹화해 병렬 fetch로 빠르게 복원됩니다.
...
프로젝트 루트의 `.md` 파일을 자동으로 스캔하고, `[[wikilink]]`를 파싱해 관계 그래프를 구성하는 Obsidian-style 파일 기반 위키입니다. 기존 DB-backed 위키(섹션 31)와 병렬로 동작하며, 파일이 곧 노드가 되므로 별도 인제스트 없이 프로젝트 문서를 LLM에 주입할 수 있습니다.

#### 동작 방식

1. 서버가 프로젝트 루트를 재귀 스캔해 모든 `.md` 파일을 발견
...
task / session / discussion 폼의 Vault 주입 섹션에서 선택:

| 모드 | 동작 |
|------|------|
| **None** (기본) | vault 주입 없음 |
| **All** | 프로젝트의 모든 `.md` 파일 내용을 주입 |
| **Selected** | 선택한 파일만 주입 |
| **Auto** | 태스크 텍스트와 매칭되는 파일을 자동 선택 후 주입 |
