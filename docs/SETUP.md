...
- **한글 IME 합성 오버레이 (데스크톱)**: xterm.js가 IME 합성 중인 텍스트를 자체 그리드에 그리지 않아 TUI redraw 시 OS candidate panel이 가려지는 문제를 보정합니다. 합성 중 텍스트가 터미널 좌하단에 작은 알약 오버레이로 항상 보이며, `pointer-events: none`이라 TUI 입력을 가리지 않습니다. 모바일은 별도의 `HangulComposer` 경로 유지.
...
| 주석(Annotation) | 헤더 펜 토글 / 컨텍스트 메뉴 | 펜(2px) / 형광펜(14px, opacity 0.4) / 지우개 / Clear all. SVG 오버레이가 zoom과 동기화. **ephemeral** — 파일 전환 또는 편집 진입 시 자동 클리어, 어디에도 저장되지 않음 |
