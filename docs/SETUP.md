...
- **메모 에디터**: 제목은 선택사항(비우면 본문 첫 줄을 제목으로). 좌상단 **크게 보기(⤢)** 로 거의 전체화면 페이지 뷰 전환. 날짜를 비우면 "날짜 없음" 백로그 메모로 저장.
- **태그**: 칩 형태로 추가/삭제(색상 자동). 헤더 아래 태그 칩으로 필터.
- **소스 레이어 토글**: 메모 / 스케줄 / 플래너 / Jira 를 켜고 끄기. 프로젝트 스케줄·플래너 항목은 읽기 전용이며 클릭 시 해당 프로젝트로 이동.
- 캘린더↔우측 패널 경계를 **드래그**해 패널 너비 조절(영속).
