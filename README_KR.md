...
자기 전에 못다 한 업무나 문득 떠오른 아이디어를 Planner나 개인 캘린더에 던져 넣고, TODO나 예약 실행으로 변환한 뒤 노트북을 닫는다. 당신이 자는 동안 Claude / Gemini / Codex가 병렬 워크트리에서 토큰 한도를 끝까지 써가며 일한다. 다음날 아침 커피 한 잔 들고 책상에 앉아 결과물을 통합적으로 살펴본다 — 마음에 드는 건 accept, 아닌 건 reject, 준비된 건 머지.
...
```
   ...
   ─────────               ────────────────────────            ─────────────────────
    캡처          →           AI가 병렬로 실행         →        통합 리뷰
   • 내 일정                  • 워크트리 격리                   • Diff 하나하나
   • Planner                 • 한도 초과 자동 복구             • 로그 하나하나
   • 위키 / TODO             • 다중 AI 토론                    • Accept / Reject / Merge
```

CLITrigger의 모든 기능은 이 루프를 지원하려고 존재한다: **캡처 → 위임 → 리뷰**.
...
<!-- screenshot: docs/images/screenshot-agenda.png (촬영 후 추가) -->
