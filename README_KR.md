...
> ### 자는 동안, 당신의 AI는 일합니다.
>
> AI가 코드를 짜는 시대, 개발자의 역할이 바뀌고 있다. 하지만 **이해 없는 바이브 코딩**은 결국 한계에 부딪힌다.
>
> CLITrigger는 자기 전 큐잉한 작업을 밤새 여러 AI 코딩 에이전트(**Claude Code · Codex · Gemini CLI**)가 각자의 git worktree에서 병렬로 처리하고 — 다음 날 아침 당신은 쌓인 diff를 보고 **accept / reject**만 하면 된다.
>
...
CLITrigger의 모든 기능 — Planner, Scheduler, 워크트리 격리, 한도 초과 자동 복구, 다중 AI 토론, 내장 Git 클라이언트 — 이 루프를 지원하려고 존재한다: **캡처 → 위임 → 리뷰**.
